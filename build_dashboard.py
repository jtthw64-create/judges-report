...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .sort-btn{padding:1px 5px;border:1px solid var(--line);border-radius:6px;background:none;color:var(--sub);font:inherit;font-size:10px;font-weight:600;cursor:pointer;white-space:nowrap}
  .sort-btn:hover,.sort-btn:focus-visible{color:var(--accent);border-color:var(--accent)}
  ...
  .skip-badge{display:inline-block;margin-left:6px;padding:1px 7px;border:1px solid var(--line);border-radius:10px;color:var(--sub);font-size:10px;font-weight:700;white-space:nowrap;vertical-align:1px}
  tr.skip td{opacity:.6}tr.skip .skip-badge,tr.skip .title{opacity:1}
  ...
  .no-link{display:inline-block;padding:4px 7px;color:var(--sub);font-size:11px;white-space:nowrap;font-style:italic}
  ...
  <h2 class="sec">우선순위</h2>
  <div class="cats" id="bds"></div>
  <div class="bar">
    <input id="q" placeholder="검색 (저자·제목·저널)…" oninput="render()">
    <button id="fAll" onclick="resetBar()">전체</button>
    <button id="fMajor" onclick="setBd('통독')">Major</button>
    <button id="fSecondary" onclick="setBd('표적')">Secondary</button>
  ...
  <div class="tablewrap"><table><thead><tr>
    <th>받음</th><th id="bdHead" aria-sort="none">우선(클릭:전환) <button class="sort-btn" type="button" onclick="toggleSort('bd')" title="우선순위순 정렬(Major 먼저)">정렬 <span id="sortMark_bd">⇅</span></button></th><th id="titleHead" aria-sort="none">자료 <button class="sort-btn" type="button" onclick="toggleSort('ti')" title="제목순 정렬">제목 <span id="sortMark_ti">⇅</span></button> <button class="sort-btn" type="button" onclick="toggleSort('yr')" title="연도순 정렬">연도 <span id="sortMark_yr">⇅</span></button></th><th id="authorHead" aria-sort="none"><button class="author-sort" type="button" onclick="toggleSort('au')">저자 <span id="sortMark_au">⇅</span></button></th><th>저널·시리즈</th><th>접근</th><th>원본 엑셀 위치</th><th>재검토 신청</th><th>코멘트</th>
...
const BD_RANK={"통독":0,"표적":1,"off-list":2};   // 정렬·칩 나열 순서(중요도 순)
...
let catFilter=null;
let typeFilter=null;
// 우선순위(통독/표적/off-list)는 예전에 statusFilter 안에 들어 있어서 '미수령만'과 동시에 쓸 수 없었다.
// 카테고리·자료종류처럼 독립 축으로 분리해 미수령+카테고리+자료종류+우선순위를 함께 적용할 수 있게 한다.
let bdFilter=null;
// 자료종류 필터가 걸린 상태에서 그 행의 종류를 바꾸면 즉시 필터 밖으로 밀려나 행이 사라진다.
// 방금 사용자가 손댄 행은 필터를 무시하고 계속 보여줘서, 원하는 종류에 닿을 때까지 연속 클릭할 수 있게 한다.
// 필터·검색을 새로 조작하면 비운다.
let recentRestype=new Set();
...
function setBd(v){bdFilter=(bdFilter===v)?null:v;recentRestype.clear();render()}
// 이 버튼 줄(전체·Major·Secondary·미수령만)은 한 묶음으로 읽히므로 '전체'는 그 줄의 선택을 모두 푼다.
// 카테고리·자료종류는 별도 줄에 있어 그대로 둔다.
function resetBar(){statusFilter={type:"all"};bdFilter=null;recentRestype.clear();render()}
// 저자·연도·제목 공통 정렬. 같은 키를 계속 누르면 오름차 → 내림차 → 해제(원래 순서)로 순환한다.
...
// '불필요'(코멘트 패널의 skip)로 표시한 항목. 확보불가와 함께 미수령 집계에서 뺀다.
function isSkip(d){const p=prof[d.id];return !!(p&&p.choice==="skip")}
// 미수령 = 아직 실제로 받아야 하는 것. 이미 받음·확보완료·확보불가·불필요는 제외한다.
// (확보불가·불필요는 "더 볼 필요 없다"고 사용자가 판정한 것이라 남은 할 일 수치를 부풀리면 안 된다.)
function isTodo(d){return !got[d.id]&&!unavailable[d.id]&&!isSkip(d)&&d.status!=="HELD_ALREADY"}
...
function cycleRestype(id,orig){
  ...
  const backToOrig=(next===orig);
  if(backToOrig) delete restypeOverride[id]; else restypeOverride[id]=next;
  recentRestype.add(id);
  // 한 바퀴 돌아 원래 종류로 되돌아온 경우 로컬에서는 override를 지우지만,
  // 백엔드에 종류명 그대로 보내면 다음 폴링(20초) 때 그 값이 override로 되살아나
  // 복원 버튼과 강조표시가 유령처럼 다시 나타난다. 되돌림은 명시적 센티널로 보낸다.
  save();syncToBackend("restype_change",{id,from:cur,to:backToOrig?"__revert_to_original__":next});render();
}
  ...
  try{
    ...
    data.rows.forEach(r=>{
      ...
      if(kind==="got"){ got[id]=(String(r.field1)==="true"); }
        ...
        // field2가 비어 있는 이벤트가 시트에 남아 있다(백엔드 v4로 restype_change를 처리하기 전에
        // 기록된 것들). 그대로 대입하면 restypeOverride[id]="" 가 되는데, 이 값은 falsy라
        // 표시되는 종류는 원본 그대로면서 `!==undefined` 검사만 통과해
        // 복원 버튼과 강조표시가 켜진 채로 남는다. 빈 값은 되돌림과 동일하게 취급한다.
        if(!r.field2||r.field2==="__revert_to_original__") delete restypeOverride[id]; else restypeOverride[id]=r.field2;
      }
...
function render(){
  ...
  ["au","yr","ti","bd"].forEach(k=>{
    const mark=document.getElementById("sortMark_"+k);
    if(mark){
      mark.textContent = sortKey===k ? (sortDir==="asc"?"▲":"▼") : "⇅";
      if(mark.parentElement) mark.parentElement.classList.toggle("on", sortKey===k);
    }
  });
  const ariaOf=k=>sortKey!==k?"none":(sortDir==="asc"?"ascending":"descending");
  document.getElementById("authorHead").setAttribute("aria-sort",ariaOf("au"));
  document.getElementById("bdHead").setAttribute("aria-sort",ariaOf("bd"));
  document.getElementById("titleHead").setAttribute("aria-sort",sortKey==="ti"||sortKey==="yr"?(sortDir==="asc"?"ascending":"descending"):"none");
  ...
  if(bdFilter==="통독")document.getElementById("fMajor").classList.add("active");
  if(bdFilter==="표적")document.getElementById("fSecondary").classList.add("active");
  ...
  const visible=DATA.filter(d=>{
    ...
    if(typeFilter&&effRestype(d)!==typeFilter&&!recentRestype.has(d.id))return false;
    if(bdFilter&&bd!==bdFilter)return false;
    if(statusFilter.type==="bd"&&bd!==statusFilter.value)return false;
    if(statusFilter.type==="todo"&&!isTodo(d))return false;
    if(statusFilter.type==="skip"&&!isSkip(d))return false;
  ...
  if(sortKey){
    ...
    const txt=(s)=>(s||"");
    // 연도는 문자열 비교하면 "1990"과 "990?" 같은 값이 뒤섞이므로 숫자만 뽑아 비교하고,
    // 연도가 없는 항목은 방향과 무관하게 항상 뒤로 보낸다.
    const yearOf=(d)=>{const m=String(d.yr||"").match(/\d{3,4}/);return m?parseInt(m[0],10):null;};
    visible.sort((a,b)=>{
      if(sortKey==="bd"){
        // 오름차 = 중요한 것 먼저(통독 → 표적 → off-list). 사전순이 아니라 중요도 순이라야 쓸모가 있다.
        const ra=BD_RANK[effBd(a)]??99, rb=BD_RANK[effBd(b)]??99;
        if(ra!==rb) return dir*(ra-rb);
        const c0=txt(a.au).localeCompare(txt(b.au),undefined,{sensitivity:"base",numeric:true});
        if(c0!==0) return c0;
        return (yearOf(a)||0)-(yearOf(b)||0);
      }
      if(sortKey==="yr"){
        const ya=yearOf(a), yb=yearOf(b);
        if(ya===null&&yb===null) return txt(a.au).localeCompare(txt(b.au),undefined,{sensitivity:"base"});
        if(ya===null) return 1;
        if(yb===null) return -1;
        if(ya!==yb) return dir*(ya-yb);
        return txt(a.au).localeCompare(txt(b.au),undefined,{sensitivity:"base"});
      }
      const key=sortKey==="ti"?"ti":"au";
      const c=txt(a[key]).localeCompare(txt(b[key]),undefined,{sensitivity:"base",numeric:true});
      if(c!==0) return dir*c;
      // 동률이면 보조 기준으로 연도를 써서 순서가 매 렌더마다 흔들리지 않게 한다.
      const ya=yearOf(a), yb=yearOf(b);
      return (ya||0)-(yb||0);
    });
  }
  visible.forEach(d=>{
    ...
    if(isSkip(d))tr.classList.add("skip");
    ...
    tr.innerHTML=`<td><input type="checkbox" class="chk" ${got[d.id]?"checked":""} onchange="toggle('${d.id}')"></td>
      ...
      <td><div class="title">${d.ti}<span class="conf conf${d.conf}">${d.conf}</span>${d.status==='HELD_ALREADY'?`<span class="held-badge">확보완료</span>`:""}${unavailable[d.id]?`<span class="unavailable-badge">확보불가</span>`:""}${isSkip(d)?`<span class="skip-badge">불필요</span>`:""}</div><div class="cite">${d.yr} <button class="mbtn cite-copy" id="cite_${d.id}" onclick="copyCitation('${d.id}')" title="파일명 규칙으로 복사">📋 파일명</button></div>${d.warn?`<div class="note warn">⚠ ${d.warn}</div>`:""}${d.info?`<div class="note info">ℹ ${d.info}</div>`:""}</td>
      ...
      <td>${d.status==='HELD_ALREADY'?`<div class="held-path" title="원본 폴더 경로">${d.heldPath||'경로 확인 필요'}</div>`:(d.link?`<a class="acc" href="${d.link}" target="_blank" rel="noopener">열기 ↗</a>`:`<span class="no-link" title="access_link 미기재 — 정제 시 누락">링크없음</span>`)}</td>
  ...
  const todo=DATA.filter(isTodo).length;
  const skipCount=DATA.filter(isSkip).length;
  ...
  const statDefs=[
    ...
    {type:"skip",label:"불필요",n:skipCount},
  ...
  document.getElementById("stats").innerHTML=statDefs.map((s,i)=>{
    // 우선순위(bd)는 독립 축이라 statusFilter가 아니라 bdFilter로 판정한다.
    const isActive = s.type==="bd" ? bdFilter===s.value
                   : statusFilter.type===s.type&&(s.value===undefined||statusFilter.value===s.value);
  ...
  [...document.getElementById("stats").children].slice(0,statDefs.length).forEach((el,i)=>{
    const s=statDefs[i];
    el.onclick=()=> s.type==="bd" ? setBd(s.value)
                 : setStatus(s.value!==undefined?{type:s.type,value:s.value}:{type:s.type});
  });

  // 선택된 카테고리·자료종류 범위 안에서의 상태별 세부 현황(상태 필터와 독립적으로 항상 전체 상태를 보여줌 — 카테고리+상태 동시 적용 가능)
  const scoped=DATA.filter(d=>(!catFilter||d.cat===catFilter)&&(!typeFilter||effRestype(d)===typeFilter)&&(!bdFilter||effBd(d)===bdFilter));
  ...
  const sSkip=scoped.filter(isSkip).length;
  const sTodo=scoped.filter(isTodo).length;
  ...
  if(bdFilter)scopeParts.push(LBL[bdFilter]||bdFilter);
  ...
  [...catsEl.children].forEach((el,i)=>{
    el.onclick=()=>{catFilter=(catFilter===cats[i])?null:cats[i];recentRestype.clear();render();};
  });
  ...
  [...typesEl.children].forEach((el,i)=>{
    el.onclick=()=>{typeFilter=(typeFilter===types[i])?null:types[i];recentRestype.clear();render();};
  });

  // 우선순위 칩. 카테고리·자료종류와 독립축이므로 함께 걸 수 있고,
  // 표시 건수는 '지금 걸려 있는 다른 축' 안에서의 수라 무엇을 고르면 몇 건이 남는지 바로 보인다.
  const bdBase=DATA.filter(d=>(!catFilter||d.cat===catFilter)&&(!typeFilter||effRestype(d)===typeFilter));
  const bdCounts={};
  bdBase.forEach(d=>{const b=effBd(d);bdCounts[b]=(bdCounts[b]||0)+1;});
  const bds=Object.keys(bdCounts).sort((a,b)=>(BD_RANK[a]??99)-(BD_RANK[b]??99));
  const bdsEl=document.getElementById("bds");
  bdsEl.innerHTML = bds.map((t,i)=>
    `<div class="cat ${bdFilter===t?'active':''}" data-idx="${i}"><b>${bdCounts[t]}</b>${LBL[t]||t}</div>`
  ).join("");
  [...bdsEl.children].forEach((el,i)=>{ el.onclick=()=>setBd(bds[i]); });
}
render();
loadFromBackend();
