...
Claude Code 토큰 한도로 인해 대시보드 기능 개발을 Codex가 이어받는다.
⚠️ **이 폴더에 이미 `2026-07-15_Codex_인수점검_보고서.md`라는 파일이 있다 — 그건 오래된 스냅샷 기준이라 낡았다(카테고리 필터·Major/Secondary 배지·백엔드 연동 등 최근 기능이 전혀 반영 안 됨). 이 문서(`00_HANDOFF_Codex_대시보드.md`)가 최신 정본이니 이것부터 읽을 것.**
